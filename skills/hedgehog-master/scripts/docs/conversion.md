...
```bash
python3 "skills/hedgehog-master/scripts/pptx_to_svg.py" \
  "examples/ppt169_kubernetes_blueprint_2026/exports/kubernetes_blueprint_2026.pptx" \
  --inheritance-mode flat \
  -o "/tmp/hedgehog-master-smoke-kubernetes"
python3 -c "import json; from pathlib import Path; report = json.loads(Path('/tmp/hedgehog-master-smoke-kubernetes/conversion-report.json').read_text()); assert report['summary'] == {'slides': 10, 'warnings': 0}, report['summary']; print('OK: 10 slides, 0 warnings')"
```
...
base = Path("/tmp/hedgehog-master-color-smoke-base.pptx")
target = Path("/tmp/hedgehog-master-color-smoke.pptx")
...
```bash
python3 "skills/hedgehog-master/scripts/pptx_to_svg.py" \
  "/tmp/hedgehog-master-color-smoke.pptx" \
  --inheritance-mode flat \
  -o "/tmp/hedgehog-master-smoke-color-tolerant"
python3 -c '
import json
from pathlib import Path

output = Path("/tmp/hedgehog-master-smoke-color-tolerant")
...
python3 "skills/hedgehog-master/scripts/pptx_to_svg.py" \
  ...
  --inheritance-mode flat \
  --strict \
  -o "/tmp/hedgehog-master-smoke-color-strict"
```
