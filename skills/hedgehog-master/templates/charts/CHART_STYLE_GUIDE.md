...
scripts = Path("skills/hedgehog-master/scripts").resolve()
...
catalog = Path("skills/hedgehog-master/templates/charts")
...
# 单文件诊断
f="your_chart.svg"
python3 "skills/hedgehog-master/scripts/svg_quality_checker.py" \
  "skills/hedgehog-master/templates/charts/$f"
...
容器卡是 Hedgehog Master 中复用率最高的视觉单元（KPI 卡、分区卡、信息卡）。下面三种图式是经过验证、与 PPTX 往返兼容的"参考实现"，新增模板优先沿用，不要发明等价但实现脏的替代。
