...
**Default — library scope**: Write `skills/hedgehog-master/templates/<kind>/<id>/` and register it in the matching discovery index.
...
|---|---|---|---|
| `library` (default) | `skills/hedgehog-master/templates/<kind_dir>/<template_id>/` | `<template_workspace>/templates/` | Run `register_template.py` against the matching global index |
...
```bash
python3 skills/hedgehog-master/scripts/pptx_template_import.py "<reference_template.pptx>"
```
...
```bash
python3 skills/hedgehog-master/scripts/svg_authoring_view.py "<import_workspace>/svg" -o "<import_workspace>/authoring-svg"
python3 skills/hedgehog-master/scripts/svg_authoring_view.py "<import_workspace>/svg-flat" -o "<import_workspace>/authoring-svg-flat"
```
...
python3 skills/hedgehog-master/scripts/extract_svg_assets.py "<import_workspace>/authoring-svg" --icons-dir "<import_workspace>/icons" --inplace --id-prefix layered --min-decoration-bytes 3000 --clean-stale
...
python3 skills/hedgehog-master/scripts/extract_svg_assets.py "<import_workspace>/authoring-svg-flat" --icons-dir "<import_workspace>/icons" --inplace --id-prefix flat --min-decoration-bytes 3000 --clean-stale
```
...
```bash
python3 skills/hedgehog-master/scripts/svg_authoring_view.py "<normalized_svg_source>" -o "<svg_analysis_workspace>/authoring-svg"
python3 skills/hedgehog-master/scripts/extract_svg_assets.py "<svg_analysis_workspace>/authoring-svg" --icons-dir "<svg_analysis_workspace>/icons" --inplace --id-prefix source --min-decoration-bytes 3000 --clean-stale
```
...
template_workspace="skills/hedgehog-master/templates/<kind_dir>/<template_id>"
...
|---|---|---|
| `library` | `skills/hedgehog-master/templates/<kind_dir>/<template_id>/` | Run the common workspace preflight; the directory name matches the final template ID used in the relevant index |
...
2. **Renames each file** using the source-order-first convention `<NNN>_<page_type>.svg`, where `<NNN>` is the source-order index zero-padded to 3 digits and `<page_type>` is typically `cover` / `toc` / `chapter` / `content` / `ending` (fall back to `content` when the type cannot be confidently classified). Examples: `001_cover.svg`, `002_toc.svg`, `003_content.svg`, ..., `050_ending.svg`.
   ...
   - Type B: derive `<page_type>` from the source filename when it follows the Hedgehog Master convention (`01_cover.svg` → `cover`, `03a_content_two_col.svg` → `content`); otherwise infer from page content or fall back to `content`
3. **Routes bundled assets through the common workspace contract** and rewrites every `<image href="...">` consistently. Keep stable source asset identity in mirror; do not rename, merge, or replace assets by semantic judgment.
   ...
   - Type B: resolve relative paths in source `<image href="...">` against the source SVG location and copy each unique asset; if the source already follows Hedgehog Master conventions (assets co-located with SVGs in the same directory), copy the whole asset set and then rewrite paths
...
```bash
python3 skills/hedgehog-master/scripts/svg_quality_checker.py "<template_workspace>/templates" --template-mode --format <canvas_format>
```
...
```bash
python3 skills/hedgehog-master/scripts/template_preview_pptx.py "<template_workspace>" --force
```
...
```bash
# For deck (default)
python3 skills/hedgehog-master/scripts/register_template.py <template_id> --kind deck

# For layout
python3 skills/hedgehog-master/scripts/register_template.py <template_id> --kind layout
```
...
>
> ```bash
> python3 skills/hedgehog-master/scripts/register_template.py --kind deck --rebuild-all
> python3 skills/hedgehog-master/scripts/register_template.py --kind layout --rebuild-all
> ```
