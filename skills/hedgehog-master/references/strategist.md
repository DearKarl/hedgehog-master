...
> 4. Search for each concept's filename in the chosen library: `ls skills/hedgehog-master/templates/icons/<chosen-library>/ | grep <keyword>`
...
> 6. **Copy each chosen icon into the project as you confirm it** — `python3 skills/hedgehog-master/scripts/icon_sync.py <project_path> <lib/name> [<lib/name> …]`. This populates `<project>/icons/<lib>/` (the set the Executor embeds from) and, more importantly, **validates existence on the spot**.
